#!/usr/bin/env python3
"""
Optimized SharePoint Incremental Backup - FIXED VERSION 2
...
class OptimizedSharePointBackupFixed:
    ...
    def _get_files_with_metadata(self, site_id: str, drive_id: str, folder_id: str = "root", depth: int = 0) -> List[FileMetadata]:
        """Get all files in a folder with metadata - FIXED with better recursion handling."""
        files = []
        ...
        try:
            ...
            while url:
                ...
                if response.status_code != 200:
                    logger.warning(f"Failed to get files from {url}: {response.status_code}")
                    break
                ...
                for item in items:
                    if 'file' in item:
                        ...
                        if depth < 20:  # Limit recursion depth to prevent infinite loops
            ...
            if depth == 0 and page_count > 0:
                logger.debug(f"Scanned {page_count} pages, {total_items} items, found {len(files)} files in drive {drive_id}")
