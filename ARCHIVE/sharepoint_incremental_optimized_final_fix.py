#!/usr/bin/env python3
"""
Optimized SharePoint Incremental Backup - FINAL FIX
Fixed version that properly handles recursion, errors, and missing metadata.
"""
...
class OptimizedSharePointBackupFinal:
    """Optimized backup using server-side metadata for change detection - FINAL FIX."""
    ...
    def _has_file_changed(self, file_meta: FileMetadata) -> bool:
        ...
        db_size = record.get('file_size', 0)
        
        # If eTag is empty in database but present in file_meta, file has changed
        if not db_eTag and file_meta.eTag:
            return True
        
        # If eTag differs, file has changed
        if file_meta.eTag != db_eTag:
            return True
        
        # If size differs, file has changed
        if file_meta.size != db_size:
            return True
    ...
    def _get_files_with_metadata(self, site_id: str, drive_id: str, folder_id: str = "root", depth: int = 0) -> List[FileMetadata]:
        """Get all files in a folder with metadata - ROBUST VERSION."""
        files = []
        ...
        try:
            ...
            while url:
                ...
                if response.status_code != 200:
                    ...
                    # Don't break, just skip this page
                    break
                    ...
                    try:
                        if 'file' in item:
                            ...
                            if depth < 50:  # Increased depth limit
                                subfolder_files = self._get_files_with_metadata(site_id, drive_id, subfolder_id, depth + 1)
                                files.extend(subfolder_files)
                            else:
                                logger.warning(f"Max recursion depth ({depth}) reached for folder: {subfolder_name}")
                    except Exception as e:
                        logger.warning(f"Error processing item in folder {folder_id}: {str(e)}")
                        # Continue with next item
                
                url = data.get('@odata.nextLink')
                params = {}
            
            if depth == 0 and page_count > 0:
                logger.info(f"Scanned {page_count} pages, {total_items} items, found {len(files)} files in drive {drive_id}")
            
            return files
            
        except Exception as e:
            logger.warning(f"Error getting files from site {site_id}, drive {drive_id}, folder {folder_id} (depth {depth}): {str(e)}")
            return []
